...
export default function Location() {
  ...
  const labelStyle =
    'text-xs font-bold text-gray-600 mb-1 uppercase tracking-wider';

  const pickupLocation = useOrderStore(
    (state) => state.orderData.pickupLocation.coordinates
  );
  const dropoffLocation = useOrderStore(
  ...
  const pickupLocationError =
    (pickupLocation[0] === 0 || pickupLocation[1] === 0) &&
    visited['pickupLocation.coordinates'];
        ...
        <div className="lg:col-span-5 flex flex-col justify-center gap-8">
          ...
          <div>
            <span className="italic text-gray-500 text-sm">
              Click on the map to set the coordinates, you can update the
              coordinates by dragging the markers.
            </span>
          </div>
        </div>
      </div>
