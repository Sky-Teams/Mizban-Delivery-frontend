...
export default function PackageInfo() {
  ...
  const packageDetails = useOrderStore(
    (state) => state.orderData.packageDetails
  );
  ...
  const weightError =
    ...
    (packageDetails.weight === 0 ||
      !VALIDATION_RULES.required(packageDetails.weight)) &&
        ...
        <div className="flex flex-col">
          ...
          <div className="relative">
            <input
              ...
              onChange={(e) =>
                updateOrderData('packageDetails.weight', e.target.value)
              }
              placeholder="0.00"
          ...
          {weightError && (
            <span className={errorStyle}>Weight must be greater than 0</span>
          )}
        </div>
        ...
        <div className="flex flex-col md:flex-row gap-6 w-full items-stretch md:col-span-2">
          <div className="flex flex-col flex-1">
            ...
            <textarea
              ...
              onChange={(e) =>
                updateOrderData('packageDetails.note', e.target.value)
              }
              id="note"
              maxLength={200}
          ...
          <div className="flex flex-col flex-1">
            ...
            <label className="flex items-center gap-3 p-5 bg-orange-50/50 border border-orange-100 rounded-xl cursor-pointer hover:bg-orange-50 transition-colors h-full min-h-[120px]">
              <input
                ...
                onChange={(e) =>
                  updateOrderData('packageDetails.fragile', e.target.checked)
                }
                className="w-5 h-5 accent-orange-600 cursor-pointer shadow-sm"
              />
              <div className="flex flex-col">
                <span className="text-sm font-bold text-gray-800 flex items-center gap-1">
                  <LuTriangleAlert className="text-orange-500" size={18} />{' '}
                  Fragile Package
                </span>
