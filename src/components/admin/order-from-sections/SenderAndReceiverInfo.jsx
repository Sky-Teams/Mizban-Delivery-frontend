...
export default function SenderAndReceiverInfo() {
  ...
  const labelStyle =
    'text-sm font-bold text-gray-700 mb-1 flex items-center gap-1';
  ...
  const senderNameError =
    !VALIDATION_RULES.required(sender.name) && visited['sender.name'];
  const senderPhoneError =
    !VALIDATION_RULES.phone(sender.phone) && visited['sender.phone'];

  const receiverNameError =
    !VALIDATION_RULES.required(receiver.name) && visited['receiver.name'];
  const receiverPhoneError =
    !VALIDATION_RULES.phone(receiver.phone) && visited['receiver.phone'];
        ...
        <div className="space-y-4">
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            <div className="flex flex-col">
              ...
              <input
                ...
                value={receiver.name}
                onChange={(e) =>
                  updateOrderData('receiver.name', e.target.value)
                }
            ...
            <div className="flex flex-col">
              ...
              <input
                ...
                onChange={(e) =>
                  updateOrderData('receiver.phone', e.target.value)
                }
                placeholder="070000000"
                className={inputStyle}
              />
          ...
          <div className="flex flex-col">
            ...
            <input
              ...
              onChange={(e) =>
                updateOrderData('receiver.address', e.target.value)
              }
              placeholder="Apartment 4, Darulaman Road, District 6, Kabul"
              className={inputStyle}
            />
            {receiverAddressError && (
              <span className={errorStyle}>Address is required</span>
            )}
          </div>
        </div>
      </div>
