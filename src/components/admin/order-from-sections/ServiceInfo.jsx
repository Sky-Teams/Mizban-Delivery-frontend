...
export default function ServiceInfo() {
  ...
  const deliveryDeadline = useOrderStore(
    (state) => state.orderData?.deliveryDeadline
  );
    ...
    <div className="bg-white p-6 rounded-xl border border-gray-100 shadow-sm mb-6">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
        ...
        <div className="flex flex-col">
          ...
          />
          {typeError && (
            <span className={errorStyle}>Please select a category</span>
          )}
        </div>
        ...
        {showScheduledFor && (
          <div>
            ...
            />
            {scheduledForError && (
              <span className={errorStyle}>Please select the date</span>
            )}
          </div>
        )}
        ...
        <div className="flex flex-col md:col-span-2 lg:col-span-1">
          ...
          <input
            ...
            onChange={(e) =>
              updateOrderData('deliveryDeadline', e.target.value)
            }
            className="p-2 bg-gray-50 border border-gray-200 rounded-lg outline-none focus:border-orange-500 focus:bg-white transition-all w-full"
          />
        </div>
