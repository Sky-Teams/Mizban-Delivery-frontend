...
export const RegistrationFileSelect = ({ label, placeholder, onChange, fileName, error }) => {
  const id = useId();
  return (
    <div className="w-full flex flex-col gap-1.5">
      {label && (
      ...
      <div className="relative">
        ...
        >
          <span className={`text-sm truncate pr-4 ${fileName ? 'text-gray-900' : 'text-gray-400'}`}>
            {fileName || placeholder}
          </span>
          <LuUpload className={`h-4 w-4 ${error ? 'text-red-500' : 'text-gray-400'}`} />
        </div>
      </div>
...
export const RegistrationInput = forwardRef(({ label, error, ...props }, ref) => {
  const id = useId();
  return (
