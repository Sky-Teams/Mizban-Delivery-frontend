      ...
      <div className="max-w-7xl mx-auto">
        <div className="flex items-center flex-wrap gap-4 justify-between mb-10">
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <p className="text-sm text-gray-500 mt-1">{t('MANAGE_NOTIFICATIONS')}</p>
            </div>
          </div>
        </div>
        ...
        <div className="bg-white rounded-2xl border border-gray-200 shadow-sm overflow-hidden p-6 space-y-3">
          {notifications.length === 0 ? (
            <p className="text-gray-400 text-center">{t('NO_NOTIFICATIONS')} </p>
          ) : (
            notifications.map((notif) => <NotifCard key={notif.id} message={notif.message} />)
          )}
        </div>
      </div>
    </div>
