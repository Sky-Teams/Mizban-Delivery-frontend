...
import { LuArrowRight, LuClock3, LuMapPin, LuPackage, LuArrowLeft } from 'react-icons/lu';
...
export default function Offers() {
  ...
  useEffect(() => {
    ...
    loadOffers();
  }, [currentPage, currentLimit, fetchOffers, setTotalPages]);
    ...
    return (
      <div className="flex min-h-14 items-center justify-center p-6">
        <div className="rounded-xl border border-red-200 bg-red-50 p-6 text-center">
          <p className="font-medium text-red-600">{t('FAILED_LOAGIN_OFFERS')}</p>
    ...
    <div className="min-h-screen bg-gray-100 p-4 sm:p-6">
      <div className="mx-auto max-w-6xl">
        ...
        <div className="mb-5 flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
          <div>
            <h1 className="mt-1 text-2xl font-bold text-gray-900">{t('YOUR_OFFERS')}</h1>

            <p className="mt-1 text-sm text-gray-500">{t('OFFERS_ASSIGNED_TO_YOU')}</p>
          </div>
        </div>
        ...
        {offers.length === 0 ? (
          <div className="rounded-xl border border-gray-200 bg-white px-6 py-14 text-center shadow-sm">
            <div className="mx-auto flex h-12 w-12 items-center justify-center rounded-full bg-gray-100">
              <LuPackage size={23} className="text-gray-400" />
            </div>

            <h2 className="mt-4 font-semibold text-gray-800">{t('NO_OFFERS')}</h2>

            <p className="mt-1 text-sm text-gray-500">{t('OFFERS_DESC_PAGE')}</p>
          </div>
        ) : (
                  ...
                  <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
                    <div className="flex min-w-0 items-center gap-3">
                      <div className="flex h-10 w-10 shrink-0 items-center justify-center rounded-lg bg-orange-50">
                        <LuPackage size={19} className="text-orange-600" />
                      </div>

                      <div className="min-w-0">
                        <p className="text-xs text-gray-400">{t('DELIVERY_OFFER')}</p>

                        <p className="truncate text-sm font-semibold text-gray-800">{offer._id}</p>
                      </div>
                    </div>
                  ...
                  {/* <div className="mt-4 rounded-lg bg-gray-50 px-3 py-3">
                    <div className="grid gap-3 sm:grid-cols-[1fr_auto_1fr] sm:items-center">
                      ...
                      <div className="hidden sm:block">
                        <LuArrowRight size={18} className="text-gray-300" />
                      </div>
                    ...
                    <div className="flex flex-wrap items-center gap-x-5 gap-y-2">
                      ...
                      <div>
                        ...
                        <div className="flex items-center gap-1">
                          <LuClock3 size={13} className="text-gray-400" />

                          <p className="text-sm text-gray-600">{formatDate(offer.offeredAt)}</p>
                        </div>
                      </div>
                    </div>
