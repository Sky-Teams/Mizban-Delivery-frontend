...
const CheckEmail = ({ email }) => {
  ...
  return (
    <AuthLayoutCard title={t('CHECK_YOUR_EMAIL')} description={t('CONFIRM_EMAIL_DSECRIPTION')} email={email}>
      {/* Button */}
      ...
      <div className="flex flex-col sm:flex-row justify-between items-center mt-1 text-xs sm:text-sm gap-2">
        <button className="text-gray-700">
          {t('DIDNT_RECIEVED_THE_LINK_QUESTION')} <span className="underline cursor-pointer">{t('RESEND')}</span>
        </button>
