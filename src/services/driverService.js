...
export const getDrivers = async (limit, page) => {
  try {
    const response = await apiClient.get(`drivers?limit=${limit}&page=${page}`).json();
...
export const createDriver = async (data) => {
  try {
    const response = await apiClient.post('drivers', { json: toDriverPayload(data) }).json();
...
export const updateDriver = async (id, data) => {
  try {
    const response = await apiClient.put(`drivers/${id}`, { json: toDriverPayload(data) }).json();
