...
export const cancelOrder = async (orderId, cancelReason) => {
    const response = await apiClient.patch(`orders/${orderId}/cancel`, { json:{"cancelReason": cancelReason} }).json()
    return response
}
...
export const assignDriver  = async(orderId, driverId)=>{
    const response = await apiClient.patch(`orders/${orderId}/assign`, {json: {"driverId": driverId}}).json()
    return response
}
