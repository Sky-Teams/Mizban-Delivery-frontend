...
const useOrderStore = create(
  immer((set, get) => ({
    ...
    getRequiredFields: (data) => {
      ...
      if (data.serviceType === SERVICE_TYPES.SCHEDULED)
    ...
    deleteItem: (id) => {
      set((draft) => {
        draft.orderData.items = draft.orderData.items.filter(
          (item) => item.id !== id
        );
      });
    ...
    resetOrderForm: () => {
      set((state) => {
        const data = state.isEditingOrder
          ? state.originalData
          : state.initialOrderDataObject;
        return {
    ...
    applyFilters: (filters, searchTerm) => {
      ...
      set((state) => ({
        filteredList: state.orders.filter((order) => {
          ...
          const matchCourier =
            !filters.courier ||
            normalize(order.courier) === normalize(filters.courier);
          const matchPaymentStatus =
            !filters.paymentStatus ||
            normalize(order.paymentStatus) === normalize(filters.paymentStatus);
          const matchStatus =
            !filters.orderStatus ||
            normalize(order.status) === normalize(filters.orderStatus);
